...
// File operation labels identify the action reported by a filesystem failure. Cleanup and expansion
// describe application operations; the remaining labels match the corresponding filesystem
// operations.
...
// FileError retains the affected path, operation classification, and original cause in a standard
// filesystem error that callers can inspect with errors.As.
